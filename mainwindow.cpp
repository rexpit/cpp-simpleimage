...
void MainWindow::filterInsertRandomNoise()
{
    ImageWidget *widget = activeImageWidget();
    if (widget != NULL) {
        widget->filterInsertRandomNoise();
    }
}
...
void MainWindow::updateMenus()
{
    ...
    if (hasMdiChild) {
        setEditMagnification(newWidget->magnification());
    }
    else {
        editMagnification->setText(QString());
    }

    if (oldWidget != newWidget) {
        if (oldWidget != NULL) {
            oldWidget->disconnect();
            ui->autoImageResizeAct->disconnect();
        }
        if (hasMdiChild) {
...
void MainWindow::setMagnificationFromEdit()
{
    ...
    if (QRegularExpression(tr("^\\d+(\\.\\d*)?%?$")).match(str).hasMatch()
        && (widget != NULL))
    {
        str.remove(QChar('%'));
