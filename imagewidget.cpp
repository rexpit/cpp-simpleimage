...
QString ImageWidget::filePath() const {
    return m_filePath;
}

QSize ImageWidget::sizeHint() const {
    return m_image.size() * m_magnification + QSize(2, 2);
}
...
void ImageWidget::paste(const QImage &image) {
    auto cmd = new CommandImageChange(this, &m_image, image, tr("貼り付け"));
    m_stack->push(cmd);
    updateImageLabel();
    autoImageResize();
}
...
bool ImageWidget::load(const QString &filePath)
{
    ...
    (void)m_image.convertToFormat(QImage::Format_RGB888, Qt::ColorOnly);
    m_stack->clear();
    m_filePath = filePath;
...
QString ImageWidget::redoText() const {
    return m_stack->redoText();
}

QString ImageWidget::undoText() const {
    return m_stack->undoText();
}

void ImageWidget::zoom(qreal m)
{
    if (m >= (qreal)common::MAGNIFICATION_MAX) {
        m = (qreal)common::MAGNIFICATION_MAX;
    }
    if (m <= (qreal)common::MAGNIFICATION_MIN) {
        m = (qreal)common::MAGNIFICATION_MIN;
    }
    m_magnification = m;
...
void ImageWidget::autoImageResize()
{
    ...
    m_magnification = qMin(qMax(qMin(magnificationX, magnificationY), (qreal)common::MAGNIFICATION_MIN), (qreal)common::MAGNIFICATION_MAX);
    updateImageLabel();
    emitZoomStateChanged();
}
...
{
    return m_magnification > (qreal)common::MAGNIFICATION_MIN;
}

qreal ImageWidget::magnification() const {
...
{
    auto cmd = new CommandFilter(new FilterGrayscale(), &m_image, tr("グレースケール化"));
    m_stack->push(cmd);
    updateImageLabel();
}
...
void ImageWidget::filterTo1Bit()
{
    ...
    if (dialog.exec() == dialog.Accepted) {
        ...
        auto cmd = new CommandFilter(filter, &m_image, tr("2値化"));
        m_stack->push(cmd);
    }
    updateImageLabel();
}
...
void ImageWidget::linearFilter()
{
    ...
    if (dialog.exec() == dialog.Accepted) {
        ...
        if (mask.isEmpty() || denominator == 0) {
            return;
        }
        auto cmd = new CommandFilter(new LinearFilter(mask, denominator, absolute), &m_image, tr("線形フィルタ"));
        m_stack->push(cmd);
    }
    updateImageLabel();
}
...
{
    auto cmd = new CommandFilter(new MedianFilter(3), &m_image, tr("メディアンフィルタ"));
    m_stack->push(cmd);
    updateImageLabel();
}
...
void ImageWidget::normFilter()
{
    ...
    if (dialog.exec() == dialog.Accepted) {
        ...
        if (maskX.isEmpty() || maskY.isEmpty()) {
            return;
        }
        auto cmd = new CommandFilter(new NormFilter(maskX, maskY), &m_image, tr("ノルムフィルタ"));
        m_stack->push(cmd);
    }
    updateImageLabel();
}
...
void ImageWidget::filterInsertRandomNoise()
{
    ...
    if (dialog.exec() == dialog.Accepted) {
        ...
        auto cmd = new CommandFilter(new FilterInsertRandomNoise(density), &m_image, tr("ランダムノイズの挿入"));
        m_stack->push(cmd);
    }
    updateImageLabel();
}
...
void ImageWidget::updateImageLabel()
{
    ...
    pixmap = pixmap.scaled(m_image.size() * m_magnification, Qt::KeepAspectRatio,
                           ((m_magnification > 1.)
